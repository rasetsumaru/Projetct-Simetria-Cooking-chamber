...
Public Class FConfigurations
    ...
    Private Sub cmdConfigManual_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdConfigManual.Click
        
		If System.Convert.ToInt32(AppService.VWGet("Controller.Controller.Application.SETTINGS.Enabled_chambers")) > 1 Then
			
			If FSelectBox.Show(Localization.GetText("@LabelForms.FSelectBox.Labels.Label004")) = DialogResult.ok Then
				
				Dim j() As Integer = {0, 0, 0}
				...
				For i As Integer = 0 To System.Convert.ToInt32(AppService.VWGet("Controller.Controller.Application.SETTINGS.Enabled_chambers")) -1
					Dim address As String = "Controller.Controller.Application.GLOBAL.Program_chamber_status<[" + i.toString() + "]>"
					j(i) = Convert.ToInt32(AppService.VWGet(address))
				Next
					
				If j(Convert.ToInt32(AppService.VWGet("ProcessSystem.ChamberIndex"))) < 1 Then
					VisiWinNET.Forms.ProjectForms.ShowChildForm("FConfigManual", "FSystem", False, VisiWinNET.Forms.FormChangeModes.ShowNewCloseActive)
				Else
					FMessageBox.Show(Localization.GetText("@LabelForms.FMessageBox.Captions.Caption001"), _
						Localization.GetText("@LabelForms.FMessageBox.Messages.Message046") + _
						(Convert.ToInt32(AppService.VWGet("ProcessSystem.ChamberIndex")) + 1).ToString + _
						Localization.GetText("@LabelForms.FMessageBox.Messages.Message047"), MessageBoxButtons.Ok)
				End If
			End If
		
		Else
		
			If System.Convert.ToInt32(AppService.VWGet("Controller.Controller.Application.GLOBAL.Program_chamber_status<[0]>")) < 1 Then
				AppService.VWSet("ProcessSystem.ChamberIndex", 1)
				VisiWinNET.Forms.ProjectForms.ShowChildForm("FConfigManual", "FSystem",False, VisiWinNET.Forms.FormChangeModes.ShowNewCloseActive)
			Else
				FMessageBox.Show(Localization.GetText("@LabelForms.FMessageBox.Captions.Caption001"), _
					Localization.GetText("@LabelForms.FMessageBox.Messages.Message048"), MessageBoxButtons.Ok)
			End If
    ...
    Private Sub cmdConfigSystem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdConfigSystem.Click
       
		If System.Convert.ToInt32(AppService.VWGet("Controller.Controller.Application.SETTINGS.Enabled_chambers")) > 1 Then
			
			If FSelectBox.Show(Localization.GetText("@LabelForms.FSelectBox.Labels.Label005")) = DialogResult.ok Then
				
				VisiWinNET.Forms.ProjectForms.ShowChildForm("FConfigSystem", "FSystem", False, VisiWinNET.Forms.FormChangeModes.ShowNewCloseActive)
